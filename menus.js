...
const menus = async function (rtm, channel) {
  const selector = '#contents > div.contentsArea.WeekMenu > div:nth-child(247) > div:nth-child(2) > table > tbody > tr:nth-child(1) > td:nth-child(n + 3) > ul > li:nth-last-child(-n + 4)';
  webScraping(url, selector).then((res) => {
    ...
    for (let i = 0; i < 5; i++) {
      ...
      for (let j = 0; j < res2.length; j++) {
        if (res2[j].match('가지') || res2[j].match('땅콩') || res2[j].match('메추리알')) {
          mealscore -= 1;
